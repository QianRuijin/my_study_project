(function ($) {
  ...
  var itemTopTmpl = '<div class="choose-content hide">' +
    ...
    $strTop = $(itemTopTmpl),
    // 底部模版字符串
    itemBottomTmpl = '<div class="bottom-content">' +
    '<div class="shop-icon">' +
      ...
      item.spus.forEach(function (_item) {
        ...
        if (_item.chooseCount > 0) {
          ...
          var price = _item.min_price * _item.chooseCount,
            row = tmpl.replace('$name', _item.name)
            .replace('$price', price)
  ...
  function addClick() {
    ...
    $strTop.on('click', '.plus', function (e) {
      var $count = $(e.currentTarget).parent().find('.count'),
        $item = $(e.currentTarget).parents('.choose-item').first(),
      ...
      renderItems();
      // 刷新当前详情页，进行联动
      window.Right.refresh($('.left-item.active').data('itemData'));
    });
    ...
    $('.clear-car').on('click', function () {
      var $chooseItem = $('.choose-item'),
        len = $chooseItem.length;
      if (!len) return;
      for (var i = 0; i < len; i++) {
        $chooseItem.eq(i).data('itemData').chooseCount = 0;
      }
      renderItems();
      // 刷新当前详情页，进行联动
      window.Right.refresh($('.left-item.active').data('itemData'));
